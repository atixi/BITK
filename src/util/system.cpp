...
const char * const BITCOIN_CONF_FILENAME = "bitcointk.conf";
...
#else
    const char* pszModule = "bitcointk";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinTK";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinTK";
#else
    // Unix
    return pathRet / ".bitcointk";
#endif
#endif
}
